...
class CustomerPortal(CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        if 'ticket_count' in counters:
            ...
            if user.user_has_groups('base.group_portal'):
                values['ticket_count'] = (
                    request.env['helpdesk.ticket'].search_count([('hide_from_portal', '=', False)])
    ...
    @route(['/my/tickets', '/my/tickets/page/<int:page>'], type='http', auth="user", website=True)
    def my_helpdesk_tickets(self, page=1, date_begin=None, date_end=None, sortby=None, filterby='all', search=None, groupby='none', search_in='content', **kw):
            ...
            #### Exclude accounts team from portal user list###
            user = request.env.user
            if user.user_has_groups('base.group_portal'):
                domain += [('hide_from_portal', '=', False)]
