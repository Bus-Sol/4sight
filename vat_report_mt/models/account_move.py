...
class AccountMove(models.Model):
    ...
    def cron_compute_field(self):
        for line in self.env['account.move'].search(
                [('invoice_line_ids', '!=', False), ('company_id', '=', 455)]).invoice_line_ids:
            if line:
                if line.tax_ids:
                    ...
                    if line.move_id.move_type == 'entry':
                        ...
                        taxes = \
                        line.tax_ids.compute_all(line.net_value, product=line.product_id, partner=line.partner_id)[
                            'taxes']
                    ...
                    if round(diff, 2) != 0:
                        if not exist_line:
                            line_id = self.env['account.move.line'].create({
                                ...
                                'gross_value': diff,
                                'vat_line_id':
                                    lines.invoice_line_ids.filtered(lambda line: line.vat_line_id != False).mapped(
                                        'vat_line_id')[0].id,
                                'name': 'VAT Rounding',
                                'account_id':
                                    lines.invoice_line_ids.filtered(lambda line: line.account_id != False).mapped(
                                        'account_id')[0].id
                            })
                        else:
                            line = self.env['account.move.line'].search(
                                [('move_id', '=', lines.id), ('is_vat_round', '=', True)])
                            line.tax_value = line.gross_value = diff
                            line.vat_line_id = \
                            lines.invoice_line_ids.filtered(lambda line: line.vat_line_id != False).mapped(
                                'vat_line_id')[0].id
