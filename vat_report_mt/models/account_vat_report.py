...
class AccountMoveLine(models.Model):
    ...
    def get_pdf(self, options, minimal_layout=True, **kwargs):
        ...
        line.append(['TOTAL', '', '','','', f"{dict_total_value['net_value']:,.2f}", f"{dict_total_value['tax_value']:,.2f}", f"{dict_total_value['gross_value']:,.2f}"])
            ...
            if line_id.vat_line_id.id != actual_vat_code:
                line.append(
                    [line_id.vat_line_id.name,'',
                     '','','', f"{dict_total_value_per_tax[str(line_id.vat_line_id.id) + '_net']:,.2f}", f"{dict_total_value_per_tax[str(line_id.vat_line_id.id) + '_tax']:,.2f}", f"{dict_total_value_per_tax[str(line_id.vat_line_id.id) + '_gross']:,.2f}"])
